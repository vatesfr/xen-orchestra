...
The feature remains administrator-only and opt-in. The shared iSCSI target currently replaces its active connection before authenticating a replacement, so an unauthenticated connection to its listener can interrupt streaming. This confirmed dependency issue must be resolved before promoting the feature; see the adversarial review in the spike report. Multi-host behavior, migration/HA, backup/snapshot interactions, Internet-latency performance, and end-to-end memory bounds remain unvalidated.
