...
beforeAll(async () => {
  jasmine.DEFAULT_TIMEOUT_INTERVAL = 150e3
})
...
describe('the VM life cyle', () => {
  ...
  describe('.restart()', () => {
    ...
    it('restarts a VM without PV drivers(clean reboot)', async () => {
      ...
      expect((await rejectionOf(xo.call('vm.restart', {
        id: hvmWithoutToolsId,
        force: false
      }))).message).toBe('VM lacks feature shutdown')
    })
  ...
  describe('.stop()', () => {
    ...
    it('stops a VM without PV drivers (clean shutdown)', async () => {
      ...
      expect((await rejectionOf(xo.call('vm.stop', {
        id: hvmWithoutToolsId,
        force: false
      }))).message).toBe('clean shutdown requires PV drivers')
    })
