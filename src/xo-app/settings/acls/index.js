      ...
      ? <Container>
        <form>
          ...
          <ButtonGroup className='p-b-1'>
            {map(TYPES, type =>
              <ActionButton tooltip={_('autoTradButton'+type)} key={type} btnStyle={isAllSelected[type] ? 'success' : 'secondary'} size='small' icon={type.toLowerCase()} handler={this._toggleAll} handlerParam={type} />
            )}
          </ButtonGroup>
          <div className='form-group'>
