.form-group(ng-if = 'ctrl.active()')
  label.col-md-1.control-label(ng-if = 'ctrl.key')
    | {{ ctrl.property.title || ctrl.key }}
    span.text-warning(ng-if = 'ctrl.isRequired()') &nbsp;*
  div(ng-class = '{"col-md-11": ctrl.key, "col-md-12": !ctrl.key}')
    ul(style = 'padding-left: 0; max-width:30em')
      li.list-group-item.clearfix(ng-repeat = 'item in ctrl.model track by $index')
        ...
        span(ng-if="!isObject(item)") {{item}}
        row(ng-if="isObject(item)" ng-repeat="(key, value) in item track by key")
          .grid-sm
            label.col-sm-2 {{key}}:
            .col-sm-10 {{value}}
    form(ng-submit = 'ctrl.add(ctrl.newItem); ctrl.newItem = ""')
      object-input(ng-if = 'ctrl.getType(ctrl.property.items) === "object"', form = 'ctrl.form', property = 'ctrl.property.items', model = 'ctrl.newItem')
      .input-group(style="max-width:30em")
        input.form-control.input-sm(ng-if = 'ctrl.getType(ctrl.property.items) === "string"', type = '{{ ctrl.isPassword ? "password" : "text" }}', ng-model = 'ctrl.newItem', ng-required = '!param.optional')
      ...
      span.help-block(ng-if = 'ctrl.getType(ctrl.property.items) !== "object"' ng-bind-html = 'ctrl.property.description | md2html')
