.tag-zone
  span
    span(ng-repeat = 'tag in ctrl.object.tags | orderBy:tag')
      span.label.label-primary
        span
          i.fa.fa-times(ng-click = 'ctrl.remove(tag)')
        | &ensp;{{tag}}
      | &ensp;
  i.fa.add-button(ng-class = '{"fa-plus": !edit, "fa-minus": edit, edit: edit}', ng-click = 'edit = !edit; newTag = ""')
    | &ensp;
  form.form-inline(ng-attr-id = 'xo-tag-{{ctrl.object.id}}',ng-submit = 'ctrl.add(newTag); newTag = ""; edit = false')
    .input-group(ng-hide = '!edit')
      input.form-control.input-sm(type = 'text', ng-model = 'newTag', form = 'xo-tag-{{ctrl.object.id}}')
      span.input-group-btn
        button.btn.btn-primary.btn-sm(type = 'submit', ng-click = '', form = 'xo-tag-{{ctrl.object.id}}') Add tag
