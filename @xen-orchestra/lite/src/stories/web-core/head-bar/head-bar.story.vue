<template>
  <ComponentStory
    ...
    :params="[
      ...
      slot('actions').help('Meant to receive VtsButton or ButtonIcon components that will trigger actions'),
  ...
  >
    <HeadBar v-bind="properties">
      ...
      <template v-if="settings.showDemoButtons" #actions>
        <VtsButton size="medium" level="primary" color="normal" :left-icon="faPlus">New VM</VtsButton>
        <VtsButton size="medium" level="secondary" color="normal" :left-icon="faPowerOff">Change state</VtsButton>
      </template>
    </HeadBar>
  </ComponentStory>
</template>
