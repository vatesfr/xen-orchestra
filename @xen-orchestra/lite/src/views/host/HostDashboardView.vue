<template>
  ...
  <div v-else class="host-dashboard-view" :class="{ mobile: uiStore.isMobile }">
    ...
    <HostDashboardCpuUsageChart class="cpu-usage-chart" :data :loading="false" />
    <HostDashboardMemoryUsageChart class="memory-usage-chart" :data :loading="false" />
    <HostDashboardNetworkUsageChart class="network-usage-chart" :data :loading="false" />
    <HostDashboardLoadAverageChart class="load-average-chart" :data :loading="false" />
  </div>
</template>
...
const { getByUuid, getStats: getHostStats } = useHostStore().subscribe()
...
watch(id, (newId, oldId) => {
  const oldHost = getByUuid(oldId)
  const newHost = getByUuid(newId)
