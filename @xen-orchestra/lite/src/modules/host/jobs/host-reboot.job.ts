...
import { hostBooleanArg } from '@/modules/host/jobs/host-boolean-args.ts'
...
export const useHostRebootJob = defineJob('host.reboot', [hostArg, hostBooleanArg], () => {
  const xapi = useXenApiStore().getXapi()
  const { t } = useI18n()
  const { isHostRunning } = useHostMetricsStore().subscribe()

  return {
    run: (host, isForceReboot) => xapi.host.cleanReboot(host.$ref, isForceReboot),
