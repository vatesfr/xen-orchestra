...
export function sortDisks(disks: DiskInfo[]): DiskInfo[] {
  const okDisks = disks.filter((d): d is DiskInfo & { ok: true } => d.ok)
  const errDisks = disks.filter((d): d is DiskInfo & { ok: false } => !d.ok)
  ...
  for (const root of roots) {
    let current: (DiskInfo & { ok: true }) | undefined = root
...
export async function listCommand(handlerUrl: string, dirPath: string, _extraArgs: string[]): Promise<void> {
  await Disposable.use(getSyncedHandler({ url: handlerUrl }), async handler => {
    ...
    const disks: DiskInfo[] = await Promise.all(
      diskPaths.map(async diskPath => {
        const filename = diskPath.slice(diskPath.lastIndexOf('/') + 1)
        try {
          return await Disposable.use(openDisk(handler, diskPath), async disk => {
          ...
          return { ok: false as const, filename, error: err instanceof Error ? err.message : String(err) }
        }
      })
    )
