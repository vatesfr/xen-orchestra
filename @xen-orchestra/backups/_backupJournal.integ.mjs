...
function writeJournalEntryAt(timestamp, entry) {
  const name = `${formatJournalTime(timestamp)}-000000-${entry.event}-${entry.vmUuid}-${entry.filename.split('/').pop()}`
  return handler.outputFile(
    `/${BACKUP_JOURNAL_DIR}/${formatJournalDay(timestamp)}/${name}`,
...
describe('backup journal', { concurrency: 1 }, () => {
  ...
  test('the entry is stored in the directory of its day, named after the time, the event and the backup', async () => {
    const path = await writeFullBackup()
    ...
    assert.equal(name.slice(0, 11), formatJournalTime(timestamp), 'the day must not be repeated in the name')
...
describe('readBackupJournal()', { concurrency: 1 }, () => {
  ...
  test('`since` only bounds the time of its own day, not of the days after it', async () => {
    ...
    await writeJournalEntryAt(midnight - 30 * 60e3, { event: 'add', vmUuid, filename: lateYesterday })
  ...
  test('skips truncated entries and unrecognized filenames without hiding their siblings', async () => {
    ...
    await handler.writeFile(
      `/${BACKUP_JOURNAL_DIR}/${day}/${formatJournalTime(Date.now())}-000000-add-${vmUuid}-truncated.json`,
      '{"ev'
    )
...
describe('readBackupJournalEvents()', { concurrency: 1 }, () => {
  ...
  test('keeps an unreadable metadata behind the watermark instead of failing the whole read', async () => {
    ...
    const { events, lastJournalRead } = await adapter.readBackupJournalEvents(0)
    ...
    const { events: retried } = await adapter.readBackupJournalEvents(lastJournalRead)
    assert.deepEqual(
