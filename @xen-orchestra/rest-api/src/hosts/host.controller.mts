...
export class HostController extends XapiXoController<XoHost> {
   ...
   *
   * Make the host join an existing pool.
   *
   * @example id "b61a5c92-700e-4966-a13b-00633f03eea8"
   * @example body { "masterAddress": "192.168.1.1", "masterUsername": "root", "masterPassword": "secret" }
   */
  @Example(taskLocation)
  @Post('{id}/actions/join_pool')
  @Middlewares([
    json(),
    acl({
      resource: 'host',
  ...
  joinPool(
    ...
    const hostId = id as XoHost['id']
    const action = async () => {
      await this.getXapiObject(hostId).$xapi.joinPool(
        body.masterAddress,
        body.masterUsername,
        body.masterPassword,
        body.force
    ...
    return this.createAction<void>(action, {
      ...
      taskProperties: {
        name: 'join pool',
        objectId: hostId,
        params: { masterAddress: body.masterAddress, masterUsername: body.masterUsername, force: body.force },
      },
    })
  }
}
