...
export function acl<T extends SupportedResource>(
  ...
  return async function _aclMiddleware(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    ...
    const paramPrivileges = privileges.map(privilege => ({ ...privilege, objects: object, user })) as Extract<
      AnyPrivilegeOnParam,
      { resource: T }
    >[]
    ...
    next(
      new ApiError('no enough privileges', 403, {
        data: {
          // No need to handle objectIds as we won't check for multiple objects here
          missingPrivileges: missingPrivileges.map(({ action, resource, objectIds }) => ({
            action,
            resource,
