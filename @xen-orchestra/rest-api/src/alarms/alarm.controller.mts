...
const ALARM_PREDICATE = CM.parse(`name:|(${ALARM_NAMES.join(' ')})`).createPredicate()
...
export class AlarmController extends XapiXoController<XoAlarm> {
  ...
  #isAlarm(message: XoMessage): boolean {
    return message.type === 'message' && ALARM_NAMES.includes(message.name)
  }
  ...
  getObject(id: XoAlarm['id']): XoAlarm {
    const maybeAlarm = super.getObject(id) as unknown as XoMessage

    if (!this.#isAlarm(maybeAlarm)) {
      /* throw */ noSuchObject(id, 'alarms')
    }
