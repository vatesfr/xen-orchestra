...
export class VmController extends XapiXoController<XoVm> {
  ...
  migrateVm(
    ...
    const action = async () => {
      ...
      const targetSr = 'srId' in body ? this.restApi.getObject<XoSr>(body.srId as XoSr['id'], 'SR') : undefined
      ...
      function srCheck(sr: XoSr) {
        if (sr.shared && sr.$container !== targetHost.$pool) {
          throw new Error('SR and host must be in the same pool') // display SR id and pool ID
        }

        if (!sr.shared && sr.$container !== targetHost.id) {
          throw new Error('SR must be reachable by the host') // display SR id and host ID
        }
      }
