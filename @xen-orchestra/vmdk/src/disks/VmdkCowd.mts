        ...
        for(let tableIndex=0; tableIndex < this.#grainDirectory.length / 4; tableIndex ++){
            ...
            for (let grainIndex = 0; grainIndex < grainTable.length / 4; grainIndex++) {
                ...
                if (grainOffset === 0) {
                  ...
                  strictEqual(grainIndex < Number.MAX_SAFE_INTEGER / GRAIN_SIZE, true, `Cowd can only handle offset up to ${Number.MAX_SAFE_INTEGER} (2^${Math.log2(Number.MAX_SAFE_INTEGER)}) -1 bytes`)
