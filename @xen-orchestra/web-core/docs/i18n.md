...
Translation files SHOULD be named with `-` when including regional variation of a language (referred as BCP47).

The reason is to avoid errors when using this language code on the frontend to compute dates, for example.
...
Since HTML is not accepted in translations, you MUST NOT use `<br>` or `\n` to insert line breaks in translation strings.

Instead, use the `{newline}` placeholder in the translation string. Then, use the [Component Interpolation](https://vue-i18n.intlify.dev/guide/advanced/component.html) feature of Vue-i18n (`I18nT` component) to render it as a `<br />` element in the template.
...
The `{newline}` placeholder in the translation string corresponds to the `#newline` slot in the `I18nT` component. At render time, it will be replaced by a `<br />` element.

> [!TIP]
> This pattern can be reused for any named placeholder that needs to render HTML. The `I18nT` component replaces `{slotName}` in the translation string with the content of the corresponding `#slotName` slot.
