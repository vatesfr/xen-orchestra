...
Keys for objects should use singular nouns and handle the pluralization with the pluralization feature of Vue-i18n.
...
To include pluralization in a translation, use `t('key', n)` where `n` is the number used to determine the plural form.
...
```json
{
  "item-count": "You have {n} item | You have {n} items"
}
```
...
```json
{
  "item-count": "Vous avez {n} article | Vous avez {n} articles"
}
```

## Handling zero in pluralization
...
```json
{
  "item-count": "You have no items | You have {n} item | You have {n} items"
}
```
...
```json
{
  "item-count": "Vous n'avez aucun article | Vous avez {n} article | Vous avez {n} articles"
}
```
