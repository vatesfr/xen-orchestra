      ...
      modalStore.addModal({
        ...
        onConfirm: async (...args: TConfirmArgs) => {
          try {
            ...
            if (result === ABORT_MODAL) {
              return
            }

            if (result instanceof AbstractModalResponse) {
              if (config.keepOpenOnAbort && !result.confirmed) {
                return
              }
              resolve(new ModalConfirmResponse(result.payload, result.confirmed))
            } else {
              resolve(new ModalConfirmResponse(result, true))
            }
        ...
        onCancel: async (...args: TCancelArgs) => {
          try {
            ...
            if (result instanceof AbstractModalResponse) {
              ...
              resolve(new ModalCancelResponse(result.payload, result.confirmed))
            } else {
              resolve(new ModalCancelResponse(result, true))
            }
