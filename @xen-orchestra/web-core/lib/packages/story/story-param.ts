import type { Widget } from '@core/packages/story/story-widget.ts'
import { boolean, choice, number, object, text } from '@core/packages/story/story-widget.ts'
import {
  faCity,
  faDesktop,
  faEllipsis,
...
import mixin, { type MixinAbstractConstructor } from '../../utils/mixin.ts'
...
export class PropParam extends mixin(BaseParam, WithWidget, WithType) {
  ...
  setRef(ref: Ref) {
    this.#ref = ref
    this.#preset = ref.value
    return this
  }

  reset() {
    if (this.#ref !== undefined) {
      this.#ref.value = this.#preset
    }
  }
