<template>
  <ComponentStory :params>
    <UiQuerySearchBar id="search-bar" v-bind="bindings" />
...
import { defineTree } from '@core/composables/tree/define-tree.ts'
import { useTreeFilter } from '@core/composables/tree-filter.composable.ts'
import { useTree } from '@core/composables/tree.composable.ts'
...
const { params, bindings } = useStory({
  events: {
    search: {
      handler: (value: string) => (filter.value = value),
      args: { value: 'string' },
    },
  },
})
