<template>
  <div class="backup-jobs-table">
    ...
    <div class="container">
      ...
      <VtsTableNew :ready="isReady" :error="hasError" :pagination-bindings sticky="right">
        <thead>
          <tr>
            <HeadCells />
          </tr>
...
const isReady = logicAnd(areSchedulesReady, areBackupLogsReady)
const hasError = logicOr(hasScheduleFetchError, hasBackupLogFetchError)
...
const { HeadCells, BodyCells } = useBackupJobColumns({
  body: (job: AnyXoBackupJob) => {
    return {
      ...
      lastRuns: r => r(getLastThreeRunsStatuses(job)),
      schedules: r => r(getTotalSchedules(job)),
