  ...
  const rxSeries = [
    {
      ...
      data: timestamps.map((timestamp, index) => ({
        timestamp,
        value: Object.values(data.stats.vifs?.rx ?? {}).reduce((sum, values) => sum + Number(values[index]), 0),
      })),
    },
  ]
  ...
  const txSeries = [
    {
      ...
      data: timestamps.map((timestamp, index) => ({
        timestamp,
        value: Object.values(data.stats.vifs?.tx ?? {}).reduce((sum, values) => sum + Number(values[index]), 0),
      })),
    },
  ]
