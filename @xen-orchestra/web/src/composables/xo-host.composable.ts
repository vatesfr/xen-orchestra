...
export function useXoHostUtils() {
  ...
  function getIfMissingPatch(hostId: XoHost['id']) {
    const { hostMissingPatches: missingPatches } = useXoHostMissingPatchesCollection({}, () => hostId)
    return missingPatches.value.length !== 0
  }
  ...
  function getNumberMissingPatches(hostId: XoHost['id']) {
    const { hostMissingPatches: missingPatches } = useXoHostMissingPatchesCollection({}, () => hostId)
    return missingPatches.value.length
  }
