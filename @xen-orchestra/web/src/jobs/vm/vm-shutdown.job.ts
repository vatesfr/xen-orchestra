...
export const useVmShutdownJob = defineJob('vm.shutdown', [vmsArg], () => {
  ...
  return {
    async run(vms: XoVm[]) {
      await Promise.all(
        vms.map(async vm => {
          const { error } = await useFetch(`/rest/v0/vms/${vm.id}/actions/clean_shutdown?sync=true`, {
            method: 'POST',
          })
    ...
    validate: (isRunning, vms: XoVm[]) => {
      ...
      if (isRunning || vms.some(vm => isVmOperatingPending(vm, VM_OPERATIONS.CLEAN_SHUTDOWN))) {
