<template>
  <div class="site-dashboard" :class="{ mobile: uiStore.isMobile }">
    ...
    <div class="row second-row">
      ...
      />
      <BackupJobsStatus class="backup-jobs-status" :backups="dashboard.backups" />
      <VmsProtection class="vms-protection" :backups="dashboard.backups" />
      <BackupIssues class="backup-issues" :issues="dashboard.backups?.issues" />
      <BackupRepository class="backup-repository" :repositories="backupRepositories" />
      <StorageRepository class="storage-repository" :repositories="storageRepositories" />
      <S3BackupRepository class="s3-backup-repository" :size="dashboard.backupRepositories?.s3?.size" />
    </div>
  </div>
</template>
