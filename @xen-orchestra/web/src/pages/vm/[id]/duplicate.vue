  ...
  <div class="duplicate">
    <form @submit.prevent="handleDuplicate()">
      <UiCard>
        ...
        >
          <UiInput v-model="duplicateVmName" required :accent="isDuplicateVmNameEmpty ? 'danger' : 'brand'" />
        </VtsInputWrapper>
...
const filteredSrs = computed(() => {
  return srs.value.filter(sr => sr.content_type !== 'iso' && sr.size > 0)
})
...
const payload = computed<DuplicateVmPayload>(() => {
  ...
  return {
    name_label: duplicateVmName.value,
    srId: selectedSr.value.id,
    ...(compressionMode.value !== 'disabled' && { compress: compressionMode.value }),
  }
})
...
async function handleDuplicate() {
  ...
  if (copyMode.value === 'fullCopy' && selectedSr.value === undefined) {
    return
  }
