...
import { useJobVmCreate } from '@/jobs/vm/vm-create.job'
...
const payload = computed(() => ({ ...vmData.value, pool: vmState.pool?.id }))
// TODO: multiple VM creation not possible in the UI for now
// Only pass a single payload
const { run, isRunning, canRun } = useJobVmCreate([payload, payload, payload, payload])
