    ...
    <form @submit.prevent="createNewVM">
      <UiCard v-if="vmState.pool" class="new-vm">
        ...
        <div v-if="vmState.new_vm_template" class="form-container">
          ...
          <div class="system">
            <div class="col-left">
              <UiInput v-model="vmState.vm_name" accent="brand" :label="t('new-vm.name')" />
              <!--        // Todo: Replace by the new select component -->
              <!--            <div class="select"> -->
              <!--              <label for="select">{{ t('tags') }}</label -->
              ...
              <!--            </div> -->
              <!--            <div class="select"> -->
              <!--              <label for="select">{{ t('boot-firmware') }}</label> -->
            ...
            <div class="col-right">
              <UiTextarea v-model="vmState.vm_description" class="description" accent="brand">
          ...
          <div class="memory">
            <UiInput v-model="vmState.vCPU" accent="brand" type="number" :label="$t('vcpu')" />
            <UiInput v-model="ramFormatted" accent="brand" type="number" :label="`${$t('ram')} (GB)`" />
            ...
            <VtsTable vertical-border>
              ...
              <tbody>
                <tr v-for="(networkInterface, index) in vmState.networkInterfaces" :key="index">
                  ...
                  <td>
                    <UiInput v-model="networkInterface.macAddress" :placeholder="t('auto-generated')" accent="brand" />
                  </td>
                  <td>
                    <UiButtonIcon
                      :icon="faTrash"
            ...
            <tbody>
              <template v-if="vmState.existingDisks && vmState.existingDisks.length > 0">
                <tr v-for="(disk, index) in vmState.existingDisks" :key="index">
                  ...
                  <td>
                    <UiInput v-model="disk.size" type="number" :placeholder="t('size')" accent="brand" />
                  </td>
                  <td>
          ...
          <UiCheckboxGroup accent="brand" class="settings">
            ...
            <UiCheckbox v-model="vmState.clone" accent="brand">{{ t('fast-clone') }}</UiCheckbox>
          </UiCheckboxGroup>
          <!--      SUMMARY SECTION -->
          ...
          <VtsResources>
            ...
            <VtsResource :icon="faDatabase" :count="totalDisks" :label="$t('sr')" />
...
const isDiskTemplate = (template: XoVmTemplate) => {
  return template && template.$VBDs.length !== 0 && template.name_label !== t('other-installation-media')
}
