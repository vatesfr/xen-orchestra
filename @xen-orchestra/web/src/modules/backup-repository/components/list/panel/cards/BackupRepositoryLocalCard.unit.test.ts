...
function mountCard(file = createFileUrl()) {
  return mount(BackupRepositoryLocalCard, {
    props: { file },
...
it('shows the path of the repository', () => {
  ...
  expect(findCardLabelledValues(wrapper)).toEqual({ [t('path')]: '/mnt/backups' })
})

it('offers to copy the path', () => {
  const wrapper = mountCard(createFileUrl({ path: '/mnt/backups' }))

  expect(findCardCopiedValues(wrapper)).toEqual(['/mnt/backups'])
})
