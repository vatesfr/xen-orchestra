...
describe('filterableVms', () => {
  test('exposes the raw RAM size from the VM memory', () => {
  ...
  test('sums the disk space from each VBD-linked VDI', () => {
    getVbdsByIds.mockReturnValue([
  ...
  test('ignores VBDs whose VDI cannot be resolved', () => {
    getVbdsByIds.mockReturnValue([
      createVbd({ VDI: 'vdi-a' as FrontXoVbd['VDI'] }),
  ...
  test('exposes the sorted IP addresses of the VM', () => {
    const filterableVm = mountFirstFilterable([
  ...
  test('recomputes when the source VMs change', () => {
    const vms = ref([createVm({ memory: { size: 1073741824, dynamic: [0, 0], static: [0, 0] } })])
    const result = mountEnhancedData(vms)

    expect(result.filterableVms.value[0].ramSize).toBe(1073741824)

    vms.value = [createVm({ memory: { size: 4294967296, dynamic: [0, 0], static: [0, 0] } })]

    expect(result.filterableVms.value[0].ramSize).toBe(4294967296)
  })
})

describe('getDisplayData', () => {
  test('formats the raw RAM size for display', () => {
  ...
  test('formats the summed disk space for display', () => {
  ...
  test('derives the VM icon from the lowercased power state', () => {
  ...
  test('maps the VM icon to the halted state when the VM is halted', () => {
