<template>
  <MenuItem v-if="canRunVmOnHost || isRunning" accent="neutral" icon="object:host" :busy="isRunning">
    {{ t('action:start-on-host') }}
    <template #submenu>
      <MenuItem v-for="host in hosts" :key="host.id" accent="neutral" @click="startOn(host)">
