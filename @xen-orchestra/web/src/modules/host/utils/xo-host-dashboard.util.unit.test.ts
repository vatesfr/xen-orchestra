...
describe('buildHostCpuUsageSeries', () => {
  ...
  it('returns an empty array when the cpu stats hold no samples', () => {
...
describe('getHostCpuUsageMaxValue', () => {
  ...
  it('keeps a value already on a hundred boundary', () => {
    expect(getHostCpuUsageMaxValue([{ timestamp: 0, value: 50 }])).toBe(100)
  })
})

describe('buildHostRamUsageSeries', () => {
  it('computes used memory as total minus free at each index', () => {
  ...
  it('propagates NaN when a free sample is missing', () => {
  ...
  it('returns an empty array when memory stats are missing', () => {
    expect(buildHostRamUsageSeries(createHostStats({ stats: { memoryFree: [40] } }))).toEqual([])
  })

  it('returns an empty array when free memory stats are missing', () => {
    expect(buildHostRamUsageSeries(createHostStats({ stats: { memory: [100] } }))).toEqual([])
  })

  it('returns an empty array when the memory stats hold no samples', () => {
...
describe('buildHostNetworkUsageSeries', () => {
  ...
  it('returns empty download and upload series when there are no pif stats', () => {
    expect(buildHostNetworkUsageSeries(createHostStats())).toEqual({ download: [], upload: [] })
  })

  it('returns empty download and upload series when the pif stats hold no samples', () => {
    const data = createHostStats({ stats: { pifs: { rx: { '0': [] }, tx: { '0': [] } } } })

    expect(buildHostNetworkUsageSeries(data)).toEqual({ download: [], upload: [] })
  })

  it('returns empty download and upload series for null data', () => {
...
describe('getHostNetworkUsageMaxValue', () => {
  ...
  it('applies a 20% headroom and rounds up to the next fifty', () => {
    const series = {
  ...
  it('picks the maximum across download and upload', () => {
    const series = {
...
describe('getHostLoadAverageMaxValue', () => {
  it('returns 10 for an empty series', () => {
    expect(getHostLoadAverageMaxValue([])).toBe(10)
  })

  it('rounds the maximum up to the next multiple of five', () => {
    expect(getHostLoadAverageMaxValue([{ timestamp: 0, value: 7 }])).toBe(10)
  })

  it('keeps a value already on a five boundary', () => {
