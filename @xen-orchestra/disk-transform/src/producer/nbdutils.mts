...
export async function connectNbdClientIfPossible(xapi: any, vdiRef: string, nbdConcurrency: number) {
  ...
  if (nbdInfos.length > 0) {
    ...
    if (poolBackupNetwork) {
      ...
      // @todo implement ipv6
      const adresses = await Promise.all(pifs.map((pifRef: string) => xapi.getField('PIF', pifRef, 'IP')))
