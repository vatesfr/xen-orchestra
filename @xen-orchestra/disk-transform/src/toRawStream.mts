...
interface ReadableWithLength extends Readable {
  length: number
}
export function toRawStream({ disk }: { disk: RandomAccessDisk }): ReadableWithLength {
  console.log({ disk })
  const stream = Readable.from(toRaw({ disk }), {
    objectMode: false,
    highWaterMark: 10 * 1024 * 1024,
  }) as ReadableWithLength
  stream.length = disk.getVirtualSize()
  return stream
}
