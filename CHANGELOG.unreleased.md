...
- [REST API/Backup] Add `backup-archives/:id/actions/mountLiveDisk`, `.../unmountLiveDisk` and `.../hydrateLiveDisk` endpoints: attach a disk of a backup as an SR, to read its content without restoring it first. Passing an `srId` caches blocks read from the backup in a disk created there, so re-reading them is local and writes are accepted; `hydrateLiveDisk` forces the whole disk into that cache upfront instead of waiting for something to read it. Without an `srId` the mount is read-only and needs no local storage, and can then target any host. Requires `iscsi.advertisedAddress` to be set to an address of this XO reachable from the hosts (PR [#10018](https://github.com/vatesfr/xen-orchestra/pull/10018))
