...
export async function set({
  ...
  if (lockingMode !== undefined) {
    await this.checkPermissions(this.apiContext.user.id, [[network?.id ?? vif.$network, 'operate']])
  }

  if (isNetworkChanged || mac) {
    const networkId = network?.id
    if (mac !== undefined && this.apiContext.user.permission !== 'admin') {
      await this.checkPermissions(this.apiContext.user.id, [[networkId ?? vif.$network, 'administrate']])
    }
    if (networkId !== undefined && this.apiContext.user.permission !== 'admin') {
      if (resourceSet !== undefined) {
        await this.checkResourceSetConstraints(resourceSet, this.apiContext.user.id, [networkId])
      } else {
        await this.checkPermissions(this.apiContext.user.id, [[networkId, 'operate']])
      }
    }
