...
export default class {
  ...
  constructor(app) {
    ...
    app.hooks.on('clean', async () => {
      await this.#roleDb.rebuildIndexes()
      await this.#privilegeDb.rebuildIndexes()
      await this.#userRoleDb.rebuildIndexes()
      await this.#groupRoleDb.rebuildIndexes()
    })
   ...
   */
  async getAclV2RolePrivileges(id) {
    const role = await this.getAclV2Role(id)
