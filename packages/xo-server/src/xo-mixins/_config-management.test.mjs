...
describe('ConfigManagement', function () {
  it('encryptCredentialDatabase forces passphrase', async function () {
    ...
    await assert.rejects(() => configManagement.exportConfig({ compress: false, entries: [] }), { message: 'Config export requires a passphrase when credential encryption is enabled.'})

    // Check that the export config works without a passphrase when encryptCredentialDatabase is true.
    await assert.doesNotReject(() => configManagement.exportConfig({ compress: false, entries: [], passphrase: 'passphrase' }))
  })
})
