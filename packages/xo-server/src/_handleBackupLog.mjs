...
import ms from 'ms'
import { computeStatusAndSortSubtasks, getStatus } from './xo-mixins/backups-ng-logs.mjs'
...
export const handleBackupLog = (taskLog, event, { app, jobName } = {}) => {
  if (event.type === 'end') {
    taskLog.status = computeStatusAndSortSubtasks(getStatus(taskLog.result, taskLog.status), taskLog)
  }
  ...
  if (app !== undefined && jobName !== undefined) {
    if (event.type === 'end' && taskLog.properties?.type === 'VM') {
      ...
      const exportTask = taskLog.tasks?.find(task => task.properties?.name === 'export' && task.tasks !== undefined)
      const transferTask =
