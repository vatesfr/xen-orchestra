...
function sendToNagios(app, vmBackupInfo) {
  app.sendPassiveCheck(
    ...
    vmBackupInfo.vmName.toLowerCase(),
    vmBackupInfo.jobName
  )
}
...
export const handleBackupLog = (
  ...
  if (event === 'end' && vmBackupInfo.hasOwnProperty(log.taskId)) {
    ...
    vmBackupInfo[log.taskId]['duration'] = moment
      .duration(log.timestamp - vmBackupInfo[log.taskId].startTimestamp)
      .humanize()
