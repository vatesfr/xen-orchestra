...
import { XoConnection } from '../_xoConnection.mjs'
import map from 'lodash/map.js'
import find from 'lodash/find.js'

const waitObjectState = (new XoConnection('http://localhost:8000')).waitObjectState
...
describe('vm', () => {
  ...
  describe('.create()', () => {
    test('creates a VM with only a name and a template', async () => {
      const templateId = getTemplateId(config.templates.debian)
      ...
      await waitObjectState(xo, vmId, vm => {
        assert.ok(vm.id instanceof 'string')
        assert.ok(vm instanceof 'object')
      })
    })
    ...
    describe('.createHVM()', () => {
      ...
      test.skip('creates a VM with the Other Config template, three disks, two interfaces and a ISO mounted', async () => {
      ...
      test.skip('creates a VM with the Other Config template, no disk, no network and a ISO mounted', async () => {
        vmId = await createVm({
          name_label: 'vmTest',
          template: templateId,
          VIFs: [],
        ...
        await waitObjectState(xo, vmId, vm => {
          assert.equal(vm.other.base_template_name, config.templates.otherConfig)
          assert.ok(vm.VIFs.length === 0)
          assert(vm.$VBDs.length === 0)
        })
      })
    })
    describe('.createPV()', () => {
      ...
      test.skip('creates a VM with the Debian 7 64 bits template, network install, one disk, one network', async () => {
      ...
      test('creates a VM with the CentOS 7 64 bits template, two disks, two networks and a ISO mounted', async () => {
  ...
  describe('.delete()', () => {
    ...
    test('deletes a VM and its snapshots', async () => {
    ...
    test('deletes a VM and its disks', async () => {
      jest.setTimeout(5e3)
      // create disk
    ...
    test.skip('deletes a vm but not delete its ISO', async () => {
      vmId = await createVmTest()
  ...
  describe('.migrate', () => {
    ...
    test('migrates the VM on an other host', async () => {
      await xo.call('vm.migrate', {
        id: vmId,
        host_id: hostId,
      })
  ...
  describe('.migratePool()', () => {
    ...
    test.skip('migrates the VM on an other host which is in an other pool', async () => {
      await xo.call('vm.migrate_pool', {
        id: vmId,
  ...
  describe('.revert()', () => {
    ...
    test('reverts a snapshot to its parent VM', async () => {
  ...
  describe('.attachDisk()', () => {
    ...
    test('attaches the disk to the VM with attributes by default', async () => {
      await xo.call('vm.attachDisk', {
        vm: vmId,
        vdi: diskId,
      })
    ...
    test('attaches the disk to the VM with specified attributes', async () => {
      await xo.call('vm.attachDisk', {
        vm: vmId,
        vdi: diskId,
        bootable: true,
  ...
  describe('.createInterface()', () => {
    ...
    test('create a VIF between the VM and the network', async () => {
    ...
    test('can not create two interfaces on the same device', async () => {
      vifId = await xo.call('vm.createInterface', {
        vm: vmId,
        network: networkId,
        position: '1',
  ...
  describe('.stats()', () => {
    ...
    test('returns an array with statistics of the VM', async () => {
