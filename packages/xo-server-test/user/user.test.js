    ...
    test('.set() :', async t => {
      withData(
        ...
        async data => {
          try {
            ...
            t.assert.equal(await xo.call('user.set', data), true)
            assert.skip(await xo.getUser(data.id), {
      ...
      withData(
        ...
        async data => {
          ...
          await testWithOtherConnection(SIMPLE_USER, xo => assert.rejects(xo.call('user.set', data))) // .rejects.toMatchSnapshot())
        }
      )
