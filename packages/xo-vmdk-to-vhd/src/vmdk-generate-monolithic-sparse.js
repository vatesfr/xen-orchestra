...
export async function generateVmdkStream(vhdStream) {
  ...
  async function* iterator() {
    for await (const item of vhdIterator) {
      switch (item.type) {
        case 'footer':
          vhdFooter = item.footer
          assert(vhdFooter.diskType === DISK_TYPES.DYNAMIC)
          break
        case 'header':
          vhdHeader = item.header
          break
        case 'parentLocator':
          // don't care for dynamic disks
          break
        case 'bat':
          {
        ...
        case 'block':
          // the grain table have been computed to be ordered and gapless
          yield item.data
          break
        default:
          throw new Error(`unhandled type of block generated by parser : ${item.type} while generating`)
      }
    }
  }
  ...
  // don't return stream while we don't have it's length
